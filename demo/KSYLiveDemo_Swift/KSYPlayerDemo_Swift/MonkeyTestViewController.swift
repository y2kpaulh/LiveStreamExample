...
class MonkeyTestViewController: UIViewController {
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "isRunning" {
            if isRunning {
                _controlButton?.setTitle("stop", for: .normal)
            }else{
                _controlButton?.setTitle("run", for: .normal)
            }
        }
    }
    ...
    private func initUI() {
        ...
        _quitButton?.setTitle("Quit", for: .normal)
        ...
        _scanButton?.setTitle("Scan code", for: .normal)

...
class KSYPlayerVC: UIViewController {
    ...
    func initUI() {
        ...
        btnRotate = addButton(with: "Rotate", action: #selector(onRotate(sender:)))
        ...
        btnMirror = addButton(with: "Mirror", action: #selector(onMirror(sender:)))
    ...
    @objc func didFinishSaving(image: UIImage, error: NSError?, contextInfo: UnsafeMutableRawPointer) {
        if let _ = error {
            let toast = UIAlertView.init(title: "￣へ￣", message: "Thumbnail capture failed！", delegate: nil, cancelButtonTitle: "determine")
            toast.show()
        }else{
            let toast = UIAlertView.init(title: "O(∩_∩)O~~", message: "Screenshots saved to phone album", delegate: nil, cancelButtonTitle: "determine")
            toast.show()
        }
    }
    ...
    @objc func updateStat(time: Timer) {
        ...
        let infoStr = String.init(format:
            ...
            "Broadcast Duration:% .1fs \n" +
            "Cache duration:% .1fs \n" +
            "Total video length% .1fs \n" +
            "cache count:% .1fs /% ld \n" +
            "Max buffering time:% .1fs \n" +
            "Speed:% 0.1f kbps \nVideo / audio rendering time:% dms /% dms \n" +
            "Time spent on HTTP connection:% ldms \n" +
            "DNS resolution time:% ldms \n" +
            "Time to first packet (after connection is established):% ldms \n" +
            "Audio buffer queue length:% .1fMB \n" +
            "Audio buffer queue duration:% .1fs \n" +
            "Amount of audio data downloaded:% .1fMB \n" +
            "Video buffer queue length:% .1fMB \n" +
            "Video buffer queue duration:% .1fs \n" +
            "Downloaded video data amount:% .1fMB \n" +
            "Total downloaded data% .1fMB \n" +
            "Decode frame rate:% .2f Display frame rate:% .2f \n",

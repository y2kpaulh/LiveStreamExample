        ...
        if !isRunning {
            ...
            if let _ = tracker ,let _ = tfDomain{
                if tracker!.start((tfDomain!.text)!) != 0 {
                    displayStr = "Failed to start the probe, please check the network or the address to be probed!"
                    displayInfo()
                    return
                }
            }
            ...
            isRunning = !isRunning
            displayStr = "Start probing......\n"
            stateStr = "Start probing......\n"
            displayInfo()
        }else{
            stopNetDiagnosis()
            if action == KSY_NETTRACKER_ACTION.PING {
                ...
                displayStr = "Stop detection, the statistics are as follows：\n"
                stateStr = "Stop detection, the statistics are as follows：\n"

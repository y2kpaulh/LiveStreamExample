...
//    }
    ...
    init(Cfg presetCfgView: KSYPresetCfgView) {
        ...
        menuNames = ["BGM", "Img/Bty", "Sound", "etc"]
    ...
    override func viewDidLoad() {
        ...
        setStreamerCfg()
        // 打印版本号信息
        print("version: \(String(describing: kit?.getKSYVersion()))")
    ...
    func onAMixerSwitch(sw: UISwitch) {
        ...
        // 静音推流(发送音量为0的数据)
            ...
            if !KSYAUAudioCapture.isHeadsetPluggedIn() {
                KSYUIVC().toast(message: "No headphones, harsh sounds when you turn on the earphones", duration: 0.3)
                sw.isOn = false
    ...
    func onBypassRecord() {
        ...
        if (miscView?.swBypassRec?.isOn)! {
            if (kit?.streamerBase.isStreaming())! && !bRec {
                ...
                let msg = "Recording can be bypassed during streaming"
    ...
    func convertToPointOfInterestFromViewCoordinates(viewCoordinates: CGPoint) -> CGPoint {
        ...
        if viewRatio > apertureRatio {
            ...
            if point.x >= blackBar && point.x <= blackBar + x2 {
                ...
                xc = point.y.remainder(dividingBy: y2)
